...
-----------------------------------------------------------------------------*/
enum commands {FIRST, BCD, BIN, CO1, CO2, DEC, FLT, HEX, MES, OCT, ROM};
...
Note that other names can befreely added to the list without any problem.
-----------------------------------------------------------------------------*/
struct codify
{
	unsigned id;
...
const char * conversion (unsigned from, unsigned to, char *str, char *val)
{
	...
	switch ( from )
	{
		...
		case BIN: x = rad_to_dec(str, 2); break;

		case CO1: x = co1_to_dec(str);    break;

		case CO2: x = co2_to_dec(str);    break;

		case DEC: x = atof(str);          break;

		case MES: x = mes_to_dec(str);    break;
	...
	switch ( to )
	{
		case BCD: return dec_to_bcd (x, val);
		case BIN: return dec_to_rad (x, 2, val);
		case CO1: return dec_to_co1(x, val);
		case CO2: return dec_to_co2(x, val);
		case DEC: return dec_to_rad (x, 10, val);
		case FLT: return dec_to_flt (x, val);
		case MES: return dec_to_mes(x, val);
		case ROM: return dec_to_rom(x);
...
int format_scan (const char *num, unsigned from, unsigned to, const char *name)
{
	...
	switch ( error )
	{
		...
		/* There are errors */
		default: fprintf(stderr, "The codify is not correct.\n"); return 1;
	}
	...
	switch (from)
	{
		...
		case MES:
			...
			if ( check_base(num, 2)	) return 1; break;
...
long double rad_to_dec (const char *num, unsigned base)
{
	...
	/* Convert deciaml part to deciaml */
	if ( strchr(num, '.') != NULL )
...
const char *dec_to_bcd (long double dec, char *bcd)
{
	...
	for (unsigned i=0; i<strlen(str); i++)
		switch ( str[i] )
		{
			...
			default : fprintf(stderr, "Unhandled exception.\n"); return NULL;
		}
...
const char *dec_to_rad (long double num, unsigned base, char *bin)
{
	...
	if ( num_dec )
	{
		...
		for (unsigned i=0; i<PRECISION; i++)
		{
			...
			{
				char buff[3];
				sprintf (buff, "%c", (unsigned)cifra-10+'A');
				strcat (bin, buff);
			}

			/* Save in 'num_dec' the decimal part of the decimal number */
			sprintf (str_dec, "%Lf", num_dec-cifra);
...
const char * binary_sum1 (char *bin)
{
	...
	unsigned *tozero = malloc ( len * sizeof (unsigned) );
...
-----------------------------------------------------------------------------*/
const char * bit_number (char *str, unsigned bit, unsigned fase)
{
